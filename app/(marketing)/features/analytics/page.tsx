    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="py-20 lg:py-32 bg-background relative overflow-hidden">
        <div className="absolute inset-0">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-success/5 rounded-full blur-[100px]" />
          <div className="absolute inset-0 grid-pattern opacity-[0.03]" />
        </div>
        
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass border-success/20 text-sm font-medium text-success mb-6">
            <BarChart3 className="w-4 h-4" />
            Analytics
          </div>
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-foreground mb-6 leading-tight tracking-tight">
            Tableaux de bord{' '}
            <span className="gradient-text">avancés</span>
          </h1>
          <p className="text-lg lg:text-xl text-foreground-secondary max-w-2xl mx-auto leading-relaxed font-light">
            Suivez les coûts de main-d'œuvre, taux de couverture et performance de votre équipe avec des analytics en temps réel.
          </p>
        </div>
      </section>
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            {[
              {
                icon: DollarSign,
                title: 'Coûts de main-d\'œuvre',
                description: 'Suivez vos coûts en temps réel, prévisions budgétaires et analyses par période.',
                color: 'success',
              },
              {
                icon: TrendingUp,
                title: 'Performance',
                description: 'Analyses de productivité, taux de couverture et optimisation des plannings.',
                color: 'accent',
              },
              {
                icon: Users,
                title: 'Gestion d\'équipe',
                description: 'Répartition des heures, disponibilités et statistiques par employé.',
                color: 'cyan',
              },
            ].map((feature, i) => {
